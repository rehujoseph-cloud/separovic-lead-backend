...
app.post('/api/leads', async (req, res) => {
  const { name, email, phone, service, message, priority, score } = req.body;
  ...
  db.run(sql, [name, email, phone, service, message || '', priority || 'medium', score || 50], async function(err) {
    if (err) {
    ...
    console.log(`✅ New lead saved: ${name} - ${service} [${priority?.toUpperCase()}]`);

    // Send SMS alert via Twilio
    try {
      const priorityEmoji = priority === 'high' ? '🔥' : priority === 'medium' ? '⚡' : '📋';
      const smsBody = `${priorityEmoji} NEW LEAD!\n\nPriority: ${priority?.toUpperCase()}\nName: ${name}\nService: ${service}\nPhone: ${phone}\nEmail: ${email}`;
